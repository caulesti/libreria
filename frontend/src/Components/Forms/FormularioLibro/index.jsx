    ...
    <main className="container mx-auto max-w-3xl py-8">
      <form onSubmit={onSubmit} className="grid gap-6 border border-slate-200 rounded-lg p-6 bg-white">
        <header className="space-y-1">
          <h1 className="text-2xl font-bold">Ingresar libro</h1>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              className={errClass(!!errors.titulo)}
          ...
          <div>
            ...
            <input
              className={errClass(!!errors.autor)}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="number"
              step="0.01"
              className={errClass(!!errors.precioVenta)}
              placeholder="25.99"
            ...
            />
            {errors.precioVenta && <p className="text-red-600 text-sm mt-1">{errors.precioVenta.message}</p>}
          </div>
          <div>
            <label className="text-sm font-semibold text-slate-700">Precio del alquiler</label>
            <input
              type="number"
              step="0.01"
              className={errClass(!!errors.precioAlquiler)}
              placeholder="4.50"
              {...register('precioAlquiler', {
                valueAsNumber: true,
                min: { value: 1, message: 'Mínimo 1' },
                max: { value: 10000, message: 'Máximo 10000' }
              })}
            />
            {errors.precioAlquiler && <p className="text-red-600 text-sm mt-1">{errors.precioAlquiler.message}</p>}
          </div>
          <div>
            ...
            <input
              type="number"
              step="1"
              className={errClass(!!errors.stockDisponible)}
              placeholder="12"
            ...
            />
            {errors.stockDisponible && <p className="text-red-600 text-sm mt-1">{errors.stockDisponible.message}</p>}
          </div>
        </div>
