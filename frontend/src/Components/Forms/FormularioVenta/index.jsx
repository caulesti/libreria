    ...
    <main className="container mx-auto max-w-3xl py-8">
      <form onSubmit={onSubmit} className="grid gap-6 border border-slate-200 rounded-lg p-6 bg-white">
        <header className="space-y-1">
          <h1 className="text-2xl font-bold">Registrar una venta</h1>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <div>
            <label className="text-sm font-semibold text-slate-700">Fecha de venta</label>
            <input
              type="date"
              className={errClass(!!errors.fechaVenta)}
              {...register('fechaVenta', { required: 'Requerida' })}
            />
            {errors.fechaVenta && <p className="text-red-600 text-sm mt-1">{errors.fechaVenta.message}</p>}
          </div>
          <div>
